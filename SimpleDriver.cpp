...
ofstream dataFile;
//string filename = "collectiveTrackData.dat";
string filename = "triTrack.dat";
//target track pos (-1, 1)
float targetTrackPos = 0.0;
const int TRACK_SENSORS = 19;
const int TRACK_POS_CHANGE = 10000;

int iterationCount = 0;

void changeTrackPos()
{
	targetTrackPos = -0.75 + 1.5 * (float) rand() / (((float) RAND_MAX));
	targetTrackPos = 0.75;
}
...
SimpleDriver::getSteer(CarState &cs)
{
    ...
    float targetAngle=(cs.getAngle()-(cs.getTrackPos() + targetTrackPos)*0.5);
...
{
	iterationCount++;
	if(iterationCount % TRACK_POS_CHANGE == 0)
		changeTrackPos();
...
SimpleDriver::init(float *angles)
{
	...
	// get random Track position for entire go
	//srand(time(NULL));
	//track pos in (-1, 1)
	changeTrackPos();
	cout << "Target track position: " << targetTrackPos << "\n";
